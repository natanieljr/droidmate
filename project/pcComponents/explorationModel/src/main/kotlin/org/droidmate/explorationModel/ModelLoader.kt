...
import org.droidmate.explorationModel.config.ModelConfig.Companion.defaultWidgetSuffix
...
open class ModelLoader(protected val config: ModelConfig, private val customWidgetIndicies: Map<P,Int> = P.defaultIndicies) {  // TODO integrate logger for the intermediate processing steps
	...
	private fun log(msg: String) = if(debugOutput){ println("[${Thread.currentThread().name}] $msg") } else {}
	...
	protected fun execute(watcher: LinkedList<ModelFeatureI>): Model {
	...
	private fun traceProcessor(channel: ReceiveChannel<Path>, watcher: LinkedList<ModelFeatureI>) = launch(context(jobName)){
...
//		logger.info("parse state $stateId")
	...
	companion object {  // FIXME watcher state restoration requires eContext.onUpdate function & model.onUpdate currently only onActionUpdate is supported
		@JvmStatic fun loadModel(config: ModelConfig, watcher: LinkedList<ModelFeatureI> = LinkedList()): Model {
