...
import org.droidmate.deviceInterface.ui.UiElementProperties
...
//	var uid: UUID = (text + contentDesc).let{debugT("gen UID ${text + contentDesc}",{ if(hasContent()) it.toUUID() else  imgId }) }
	...
	val idHash get() = properties.idHash
					...
					computeId(w, wImg, true)
						...
						.let { widgetIdPair ->  // (uid,imgId)
							...
							if (wImg != null && config[ConfigProperties.ModelProperties.imgDump.widgets] && (!config[ConfigProperties.ModelProperties.imgDump.widget.onlyWhenNoText] ||
											(config[ConfigProperties.ModelProperties.imgDump.widget.onlyWhenNoText] && w.content() == "" ) ) &&
									( config[ConfigProperties.ModelProperties.imgDump.widget.interactable] && w.actable ||
											config[ConfigProperties.ModelProperties.imgDump.widget.nonInteractable] && !w.actable ) )
