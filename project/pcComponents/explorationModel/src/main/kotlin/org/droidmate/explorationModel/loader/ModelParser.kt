...
object ModelParser{
	@JvmOverloads @JvmStatic fun loadModel(config: ModelConfig, watcher: LinkedList<ModelFeatureI> = LinkedList(),
...
//			parentJob.joinChildren() } // wait until all traces were processed (the processor adds the trace to the model)
	...
	private fun traceProcessor(channel: ReceiveChannel<Path>, watcher: LinkedList<ModelFeatureI>) = CoroutineScope(newContext(jobName)).launch{
