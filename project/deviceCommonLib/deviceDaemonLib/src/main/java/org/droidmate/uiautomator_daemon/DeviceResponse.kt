...
//		}
		...
		fun create(uiHierarchy: Deferred<List<WidgetData>>, deviceModel: String, displayWidth: Int, displayHeight: Int, screenshot: ByteArray, width: Int, height: Int): DeviceResponse {
			val widgets = runBlocking{uiHierarchy.await()}
			return DeviceResponse("TODO parse widget list if we need the XML or debug property" +
					", because the dump would waste ~200ms", widgets.last().packageName, widgets, androidLauncher(deviceModel),
					displayWidth, displayHeight, screenshot, width, height)
		}
	}
