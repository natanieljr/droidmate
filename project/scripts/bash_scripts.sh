...
function run {
	gradle :project:pcComponents:core:run -PjvmArgs="-ea -DLOGS_DIRECTORY=./logs" -Pargs="-command=record -guiExplorationClicks=1 -forceInstrumentation -displayConfig -resourcesDirPath=./src/main/resources -artifactsDirPath=./artifacts"
}
