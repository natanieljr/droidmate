...
def StartLogcatIfNecessary(device_id, adb_cmd, base_dir, apk_name):
  ...
  logging.info('Starting logcat %d for device %s', logcat_num,
               device_id)
  logcat_filename = apk_name+'-logcat_%s_%03d' % (device_id, logcat_num)
...
def main(base_dir, apk_name, device_index, adb_cmd='adb'):
  ...
  try:
    ...
    while True:
      ...
      device_id = GetAttachedDevices(adb_cmd)[int(device_index)]
      ...
      for device in devices:
        ...
        StartLogcatIfNecessary(device, adb_cmd, base_dir, apk_name)
...
if __name__ == '__main__':
  ...
  print 'Usage: %s <reports_folder> <apk_name> <device_index> [<adb_binary_path>]' % sys.argv[0]