...
class ExplorationOutput2(private val list: MutableList<AbstractContext> = ArrayList()) : MutableList<AbstractContext> by list {
    companion object {
        private val log = LoggerFactory.getLogger(ExplorationOutput2::class.java)
        private const val serialVersionUID: Long = 1

        @JvmStatic
        fun from(storage: IStorage2): List<AbstractContext> {
            return storage.getSerializedRuns2().map {
                val apkout2 = storage.deserialize(it) as AbstractContext
