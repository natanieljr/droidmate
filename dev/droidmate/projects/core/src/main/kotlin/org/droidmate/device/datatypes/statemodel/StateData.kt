...
//      isSelectAHomeAppDialogBox,isUseLauncherAsHomeDialogBox)
	...
	private val lazyIds: Lazy<Pair<UUID,UUID>> =
			lazy({widgets.fold( Pair( emptyUUID, emptyUUID), { (id, configId), widget ->
  ...
  val configId:UUID by lazy{ lazyIds.value.second }
	...
	val iEditId: UUID by lazy{ //lazyIds.value.third
...
//    )
    ...
    @JvmStatic fun fromFile(widgets: Set<Widget>): StateData = StateData(widgets//.sortedBy { it.uid.toString() }
	  ...
	  @JvmStatic val emptyState: StateData by lazy{ StateData(emptySet()) }
	  ...
	  @JvmStatic fun computeIds(widgetIds:List<UUID>,editIds:List<UUID>):Pair<UUID,UUID> =
